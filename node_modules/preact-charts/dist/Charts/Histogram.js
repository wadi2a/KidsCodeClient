    ...
    render({ name, margin, x, data, ticks, barColour, barOutline }, { height, width, innerHeight, innerWidth }) {
        ...
        return (h("svg", { ref: (svg) => this.chartSVG = svg, class: name, height: height, width: width },
            h("g", { transform: `translate(${margin.left}, ${margin.top})` },
                ...
                barWidth &&
                    bins.map((bin) => h("rect", { "fill-opacity": '1', "stroke-width": '1', x: '1', width: xScale(bin.x1) - xScale(bin.x0), height: innerHeight - yScale(bin.length), transform: `translate(${xScale(bin.x0)}, ${yScale(bin.length)})`, fill: barColour, stroke: barOutline })))));
    }
    componentDidMount() {
        this.resizeChart();
